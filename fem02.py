...
def fem02(Diametros):
    ...
    # #Força destribuida no tubo do pedal
    # f[49]=-267
    # f[55]=-267
    ...
    for i in range(0, len(deslocamento), 6):
        bloco = deslocamento[i:i+3]
        resultado.extend(bloco)
        selecionados.update(bloco)

    restante = [x for x in deslocamento if x not in selecionados]
    
    desloc=max(resultado)
    rot=max(restante)
    desloc=max(deslocamento[:3])
    rot=deslocamento[3]
    return Tensao_maxima, desloc, rot
